...
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	BinaryTree<ElemType> &operator=(const BinaryTree<ElemType>& t);
		// 赋值运算符重载
	int DegreeOfTree(BinTreeNode<ElemType> *r) //求孩子兄弟树的度
	{
		if (r == NULL) return 0;
		else {
			int degree = 0, d = 0;
			BinTreeNode<ElemType> *p;
			for (p = r->leftChild; p != NULL; p = p->rightChild)
			{
				d++;
				int dd = DegreeOfTree(p);
				degree = (degree > dd) ? degree : dd;
			}
			return (d > degree) ? d : degree;			
		}
	}
	int Height(BinTreeNode<ElemType> *r)//求孩子兄弟树的高度
	{
		if (r == NULL) return 0;
		else {
			int Left, Right;
			Left = Height(r->leftChild) + 1;
			Right = Height(r->rightChild) + 0;
