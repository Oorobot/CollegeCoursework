...
int main(void)
{
	...
	//char pre[] = { 'A','B','E','F','C','D','H','G' };//孩子兄弟链表树先序
	//char in[] = { 'E','F','B','C','H','G','D','A' };//孩子兄弟链表树中序
	int n = 9;		// 结点个数
	//int n = 8;
	BinaryTree<char> bt;
    char c = 'x', e;
    ...
    while (c != '0')	{
        ...
        cout << endl << "8. 求二叉树的高度.";
        cout << endl << "9. 显示二叉排序树.";
		cout << endl << "a.求孩子兄弟链表树的度。";
		cout << endl << "  求孩子兄弟链表树的高度。";
		cout << endl << "0. 退出";
		cout << endl << "选择功能(0~7):";
		cin >> c;
		switch (c) 	{
		    case 'a':
				cout << endl << "孩子兄弟树的度：" << bt.DegreeOfTree(bt.GetRoot()) << endl;
				cout << endl << "孩子兄弟树的高度：" << bt.Height(bt.GetRoot()) << endl;
		    case '1':
            	cout << endl << "输入被插入元素的值:";
			    cin >> e;
			    p = bt.Find(e);
