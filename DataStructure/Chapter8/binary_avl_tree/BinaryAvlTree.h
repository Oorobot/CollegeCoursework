...
public:
	...
	int bfGetHeight() const;
	void bfGetHeight(const BinAVLTreeNode<ElemType>* r, int& H) const;
...
template <class ElemType>
int BinaryAVLTree<ElemType>::bfGetHeight() const
{
	int H = 0;
	bfGetHeight(root, H);
	return H;
...
template <class ElemType>
void BinaryAVLTree<ElemType>::bfGetHeight(const BinAVLTreeNode<ElemType> * r, int& H) const
{
	if (r == nullptr);
	else {
		if (r->bf < 0) {
			H = H + abs(r->bf) + 1; 
			bfGetHeight(r->rightChild, H);
		}
		else {
			H = H + abs(r->bf) + 1;
			bfGetHeight(r->leftChild, H);
		}
	}
}
template <class ElemType>
