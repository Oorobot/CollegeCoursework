...
public:
	...
	void Reverse()    //倒置
	{
		...
		{
			temp = elems[i]; elems[i] = elems[j]; elems[j] = temp;
		}
	}

	void DeleteDuplicateElem()        //删除多出的重复元素
	{
		for (int i = 0; i <= length - 1; i++)
		{
			for (int j = i + 1; j <= length - 1; j++)
				if (elems[i] == elems[j]) {
					DeleteElem(j + 1, elems[j]); j--;
				}
		}
	}

	void Sort(bool a = 1)   //排序，1时升序，0时降序。
	{
		ElemType temp;
		if (a != 0)
		{
	...
	void Merge(SeqList b)
	{
		int i = this->length;
		this->length = this->length + b.length;
		this->maxLength = this->maxLength + this->maxLength;
		for (int j = 0; j <= b.length - 1; j++, i++)
			this->elems[i] = b.elems[j];
		this->Sort(1);
	}

	Status DeleteRange(ElemType s, ElemType t)
	{
		if (s >= t || length == 0)  return RANGE_ERROR;
		else
		{
			for (int i = 0; i < length; i++)
				if (elems[i] > s && elems[i] < t) {
					DeleteElem(i + 1, elems[i]); i--;
				}
			return SUCCESS;
		}
	}
