...
public:
	...
	Status DeleteE(ElemType &e)
	{
		Node<ElemType> *p = head;
		int flag = 0;
		for (Node<ElemType> *q = p->next; q != NULL && length > 1; q = p->next)
		{
			if (q->data == e) {
				p->next = q->next; delete q; length--; flag = 1;
			}
			else flag = 0;
			if (flag == 0) p = p->next;
		}
		if (length == 1 && head->next->data == e) {
			p = head->next;
			head->next = NULL;
			delete p;
		}
	...
	/*{
		Node<ElemType> *p = head;
		int flag = 0;
		for (Node<ElemType> *q = p->next; q != NULL && p->data <= e; q = p->next)
		{
			if (q->data == e) {
				p->next = q->next; delete q; length--; flag = 1;
			}
	...
	void merge10(LinkList L)
	{
		...
		int i = 1;
		for (p = p->next, l = l->next; p != nullptr; p = p->next, i++)
		{
