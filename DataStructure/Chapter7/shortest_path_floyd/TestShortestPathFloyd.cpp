...
int main(void)
{
    try									// 用try封装可能出现异常的代码
	{
		...
		int m[6][6] = {
			...
			{infity,  infity,	      2,     infity,       10,    infity},
			{infity,  infity,    infity,          3,  infity,    infity},
			{infity,  infity,    infity,     infity,       2,    infity},
		...
		longestPathFloyd(net, path, dist);
		DisplayPathAndDist(net, path, dist);// 显示所有路径 
		TheLongestPath(net, path, dist);
		cout << endl << endl;
