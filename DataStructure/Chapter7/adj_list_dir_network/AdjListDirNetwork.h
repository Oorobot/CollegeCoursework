...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	bool Route(int v1, int v2);//判断是否存在vi到vj的路线
};
...
template <class ElemType, class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::Route(int v1, int v2)
{
	if (v1 < 0 || v1 >= vexNum)
		throw Error("v1不合法!");        // 抛出异常
	if (v2 < 0 || v2 >= vexNum)
	...
	for (AdjListNetworkArc<WeightType> *p = vexTable[v1].firstarc; p != NULL; p=p->nextarc) {
		if (GetTag(p->adjVex) == UNVISITED && p->adjVex != v2) Route(p->adjVex, v2);
		else if (p->adjVex == v2) return true;
	}	
	return false;
}
